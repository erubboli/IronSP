  ...
  module SassCompiler
    class << self
        ...
        unless watching?
          @handler = Proc.new do |sender, args|
            if (args.event == "ItemUpdated" || args.event == "ItemAdded")
              ...
              if ([:scss, :sass].include? extension)
                compile_files
              end
            end
          end
      ...
      def compile_file url
        ...
        begin
          engine = Sass::Engine.new content, :syntax => extension, :load_paths => [".", "app/assets/stylesheets"], :cache => false
      ...
      def compile_files
        sass_files = hive.files.select do |x|
          x[/^app\/assets\/stylesheets\/\w+\.(scss|sass)/]
        end
